...
9.3章でのセグメントツリーの演算は木の\emph{下から上}へ歩くように実装してきました。例えば以下のように範囲和を計算しました。
...
範囲更新には2種類あり全てのノードの値を\emph{増加}させたいか全てのノードの値に\emph{更新}を行いたいかです。
...
$s$は範囲内の値の合計を表して$z$は範囲内のすべての値を$z$だけ増やすべきという遅延更新の値です。
...
あるノードの範囲 $[x, y]$ が完全に $[a, b]$ に属する場合、そのノードの $s$ 値を総和に加えます。
...
値$s$は範囲内の要素の和であり値$z_0,z_1,\ldots,z_k$は遅延更新に対応する多項式の係数になります。
...
\end{lstlisting}
\texttt{value}はノードの値，$[x, y]$は対応する範囲、\texttt{left}と\texttt{right}はその子のツリーを指します。
...
各ノードにデータ構造を割り当てたセグメントツリーを構築して任意の要素$x$が対応する範囲に何回現れるかをクエリできます。
この木を用いて範囲に属するノードからの結果をマージすることで問い合わせに対する答えが分かります。
