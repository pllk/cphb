...
各ノードの出現回数が せいぜい1回であるとき、パスは\texttt{単純}と呼ばれます。
...
なぜなら、各辺はちょうど2つのノードの度数を1つずつ増加させるからである。
このため、次数の和は常に偶数となります。
...
隣接行列表現の欠点として行列が$n^2$の要素を含んで
通常はそのほとんどが$0$であることである。
